...
If this sounds familiar, you're not alone. Something strange is happening to developers in 2026: **AI coding tools are triggering the same dopamine loops as slot machines**, and the tech industry is sleepwalking into it.
...
I've caught myself in the same loop. You fire off a prompt, and while the agent works, you're already thinking about the next one. There's no natural stopping point. Unlike writing code manually, where fatigue creates friction, AI coding feels effortless. The bottleneck shifts from your fingers to your imagination, and imagination doesn't get tired at 1 AM — it gets *more creative*.

The result? Developers who used to close their laptops at a reasonable hour are now routinely coding until 2, 3, 4 AM. Not because the work demands it, but because the loop demands it.

## The vibe coding gold rush

Meanwhile, the industry is pouring gasoline on the fire. Garry Tan, Y Combinator's CEO, has been one of the loudest evangelists for what Andrej Karpathy coined "vibe coding" — building software by feel, guided by AI rather than deep technical understanding.

According to TechCrunch, roughly a quarter of YC's recent batch has codebases that are *almost entirely AI-generated*. Tan has called this a superpower for startups, describing "20x companies" where tiny teams automate everything internally with AI agents.

He's not wrong about the productivity gains. But nobody in that conversation is talking about what happens to the humans in the loop. When your entire startup is built on the dopamine of watching agents ship features, what happens when the agents hallucinate at 3 AM and you're too wired to notice?
...
- **Hard shutdown time.** I set a non-negotiable 11 PM cutoff. Phone alarm, laptop closed, no exceptions. The work will be there tomorrow — and I'll be sharper for it.
- **Batch, don't stream.** Instead of watching every agent run in real time, I queue up tasks and review results in batches. This breaks the real-time reinforcement loop.
- **Track the hours, not the output.** It's easy to justify "just one more prompt" when you're shipping features. But shipping features at 3 AM with degraded judgment creates technical debt you'll pay for later.
- **Talk about it.** The first step is admitting this is a real phenomenon, not a humble brag. When someone says they "couldn't stop coding all night," that's not dedication — it might be a problem.
...
The slot machine in your terminal doesn't care if you sleep. But you should.